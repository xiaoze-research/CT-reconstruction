...
When running inside the Code Ocean capsule, `data/` corresponds to the `/data`
mount and generated outputs go to `/results` instead of `outputs/`; the capsule
entry point (`run`) handles both paths automatically.
