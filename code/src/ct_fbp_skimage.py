...
from projection_io import find_projection_files, load_projection_stack
...
def reconstruct_volume(args: argparse.Namespace) -> np.ndarray:
    # Peek at the first projection to size the layer window, then load only
    # the detector rows that will actually be reconstructed.
    probe_paths = find_projection_files(
    ...
    if not probe_paths:
        raise ValueError(f"No projection images found in {args.input_dir}")
    with Image.open(probe_paths[0]) as first:
    ...
    for index, z in enumerate(range(start, stop), start=1):
        sinogram = projections[:, z - start, :].T
