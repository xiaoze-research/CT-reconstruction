...
(In the Code Ocean capsule, input data is mounted at `/data` and outputs are
written to `/results`; the capsule's `run` script handles both paths.)
...
python src\astra_cone_beam_fdk_cuda.py `
  ...
  --center-offset-px <CENTER_OFFSET_PIXELS>
```

`--filter-type` and `--center-offset-px` are optional: omit them to use the
ASTRA library default filter and no detector-centre correction.
